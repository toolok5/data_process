...
def process_files(root):
    ...
    for file_path in file_paths:
        ...
        try:
            # 指定只读取前18列的数据
            df = pd.read_csv(file_path, encoding='gbk', usecols=range(18), parse_dates=['创建时间'], date_format='%Y-%m-%d %H:%M:%S')
        ...
        # 填充 10-14 列的空值为 ""
        columns_to_check = df.columns[9:14]  # 假设列从0开始计数
        ...
        # 删除 "执行步骤顺序" 列（如果存在）
        if '执行步骤顺序' in df.columns:
            df = df.drop(columns=['执行步骤顺序'])
    ...
    if aggregated_data:
        ...
        for week_key, groups in aggregated_data.items():
            file_name = f"集中参数数据处理-邻区参数{week_key}.csv"
